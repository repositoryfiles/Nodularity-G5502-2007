...
marumi_ratio = 0.45 #形状ⅤとⅥと判定する丸み係数のしきい値（JIS G5502 (2007)において、この値はJISの規定値ではないことに注意！）
...
def main():
    ...
    for filename in filenames:
        ...
        for i, cnt in enumerate(contours1): 
            ...
            # JIS G5502 (2007)による形状分類
            if marumi >= marumi_ratio:
                num_graphite += 1
                cv2.drawContours(img_color_JIS, contours1, i, (0, 0,255), 2) #赤
                    
        # 球状化率（JIS G5502 (2007)）
        nodularity_old_JIS.append(num_graphite / len(contours1) * 100)
        
        # 結果のファイル保存（画像ファイル名＋_resで同じフォルダに保存）
        src = filename
        idx = src.rfind(r'.')
        result_filename = src[:idx] + "_nodularity_old_JIS_(marumi = " + str(marumi_ratio) + ")." + src[idx+1:]
        cv2.imwrite(result_filename, img_color_JIS)
    ...
    if output_file != "":
        ...
        print("ファイル名, 球状化率_G5502(2007)(%)", file = output_file)
        for i in range(len(filenames)):
            print("{}, {:.2f}" .format(filenames[i], nodularity_old_JIS[i]), file = output_file)
